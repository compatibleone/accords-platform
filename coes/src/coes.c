...
struct	broker_configuration Coes = {
	0,
	0,
	0,0,0,
	(char *) 0,
...
private	int	banner()
{
	printf("\n   CompatibleOne Elasticity Services COES : Version 1.0a.0.02");
		...
		else if ( *aptr == '-' )
		{
			...
			case	'-'	:
				if (!( strcmp( aptr, "auto" ) ))
					...
					Coes.tls = allocate_string( argv[++argi] );
				else if (!( strcmp( aptr, "config" ) ))
				{
					Coes.config = allocate_string( argv[++argi] );
					coes_load();
				}
				else if (!( strcmp( aptr, "domain" ) ))
					Coes.domain = allocate_string( argv[++argi] );
				else if (!( strcmp( aptr, "publisher" ) ))
					Coes.publisher = allocate_string( argv[++argi] );
				else if (!( strcmp( aptr, "identity" ) ))
					Coes.identity = allocate_string( argv[++argi] );
				else if (!( strcmp( aptr, "user" ) ))
					Coes.user = allocate_string( argv[++argi] );
				else if (!( strcmp( aptr, "password" ) ))
					Coes.password = allocate_string( argv[++argi] );
				else if (!( strcmp( aptr, "resthost" ) ))
					Coes.resthost = allocate_string( argv[++argi] );
				else if (!( strcmp( aptr, "restport" ) ))
					Coes.restport = atoi( argv[++argi] );
				else if (!( strcmp( aptr, "threads" ) ))
					Coes.threads = _REST_THREAD_WORKER;
				else	break;
				continue;
			}
			status = 30;
