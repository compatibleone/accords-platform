...
{
	load_accords_configuration( &Configuration, "paasprocci" );
	return;
}
...
private	int	paasprocci_operation( char * nptr )
{
	...
	/* ---------------------------------------------*/
	if (!( optr = occi_add_action( optr, _CORDS_START, "", start_paas)))
		return( 27 );
	else if (!( optr = occi_add_action( optr, _CORDS_STOP," ", stop_paas)))
		return( 27 );
	else if (!( optr = occi_add_action( optr, _CORDS_SAVE, "", save_paas)))
		return( 27 );
