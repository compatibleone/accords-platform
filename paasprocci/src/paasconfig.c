/* -------------------------------------------------------------------- */
/*  ACCORDS PLATFORM                                                    */
/*  (C) 2011 by Iain James Marshall (Prologue) <ijm667@hotmail.com>     */
/* -------------------------------------------------------------------- */
/* Licensed under the Apache License, Version 2.0 (the "License"); 	*/
/* you may not use this file except in compliance with the License. 	*/
/* You may obtain a copy of the License at 				*/
/*  									*/
/*  http://www.apache.org/licenses/LICENSE-2.0 				*/
/*  									*/
/* Unless required by applicable law or agreed to in writing, software 	*/
/* distributed under the License is distributed on an "AS IS" BASIS, 	*/
/* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or 	*/
/* implied. 								*/
/* See the License for the specific language governing permissions and 	*/
/* limitations under the License. 					*/
/* -------------------------------------------------------------------- */

/* STRUKT WARNING : this file has been generated and should not be modified by hand */
#ifndef _paasconfig_c_
#define _paasconfig_c_

#include "element.h"

#include "paasconfig.h"

/*	----------------------------------------	*/
/*	l i b e r a t e _ p a a s _ c o n f i g 	*/
/*	----------------------------------------	*/
public struct paas_config * liberate_paas_config(struct paas_config * sptr)
{
	if ( sptr )
	{
		if ( sptr->id )
			 sptr->id = liberate(sptr->id);
		if ( sptr->name )
			 sptr->name = liberate(sptr->name);
		if ( sptr->description )
			 sptr->description = liberate(sptr->description);
		if ( sptr->host )
			 sptr->host = liberate(sptr->host);
		if ( sptr->user )
			 sptr->user = liberate(sptr->user);
		if ( sptr->password )
			 sptr->password = liberate(sptr->password);
		if ( sptr->subscription )
			 sptr->subscription = liberate(sptr->subscription);
		if ( sptr->tenant )
			 sptr->tenant = liberate(sptr->tenant);
		if ( sptr->version )
			 sptr->version = liberate(sptr->version);
		sptr = liberate( sptr );
	}
	return((struct paas_config *) 0);

}

/*	----------------------------------	*/
/*	r e s e t _ p a a s _ c o n f i g 	*/
/*	----------------------------------	*/
public struct paas_config * reset_paas_config(struct paas_config * sptr)
{
	if ( sptr )
	{
		sptr->id = (char*) 0;
		sptr->name = (char*) 0;
		sptr->description = (char*) 0;
		sptr->host = (char*) 0;
		sptr->user = (char*) 0;
		sptr->password = (char*) 0;
		sptr->subscription = (char*) 0;
		sptr->tenant = (char*) 0;
		sptr->version = (char*) 0;
		sptr->port =  0;
		sptr->state =  0;
	}
	return(sptr);

}

/*	----------------------------------------	*/
/*	a l l o c a t e _ p a a s _ c o n f i g 	*/
/*	----------------------------------------	*/
public struct paas_config * allocate_paas_config()
{
	struct paas_config * sptr;
	if (!( sptr = allocate( sizeof( struct paas_config ) ) ))
		return( sptr );
	else	return( reset_paas_config(sptr) );
}

/*	----------------------------------	*/
/*	x m l i n _ p a a s _ c o n f i g 	*/
/*	----------------------------------	*/
public int xmlin_paas_config(struct paas_config * sptr,struct xml_element * eptr)
{
	struct xml_element * wptr;
	if (!( eptr )) return(0);
	if (!( sptr )) return(0);
	for ( wptr=eptr->first; wptr != (struct xml_element *) 0; wptr=wptr->next)
	{
		if (!( strcmp(wptr->name,"id") ))
		{
			if ( wptr->value ) { sptr->id = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"name") ))
		{
			if ( wptr->value ) { sptr->name = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"description") ))
		{
			if ( wptr->value ) { sptr->description = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"host") ))
		{
			if ( wptr->value ) { sptr->host = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"user") ))
		{
			if ( wptr->value ) { sptr->user = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"password") ))
		{
			if ( wptr->value ) { sptr->password = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"subscription") ))
		{
			if ( wptr->value ) { sptr->subscription = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"tenant") ))
		{
			if ( wptr->value ) { sptr->tenant = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"version") ))
		{
			if ( wptr->value ) { sptr->version = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"port") ))
		{
			if ( wptr->value ) { sptr->port = atoi(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"state") ))
		{
			if ( wptr->value ) { sptr->state = atoi(wptr->value); }
		}
	}
	return(0);

}

/*	------------------------------------------	*/
/*	r e s t _ o c c i _ p a a s _ c o n f i g 	*/
/*	------------------------------------------	*/
public int rest_occi_paas_config(FILE * fh,struct paas_config * sptr,char * prefix, char * nptr)
{
	struct xml_element * wptr;
	if (!( sptr )) return(0);
	fprintf(fh,"POST /%s/ HTTP/1.1\r\n",nptr);
	fprintf(fh,"Category: %s; scheme='http://scheme.%s.org/occi/%s#'; class='kind';\r\n",nptr,prefix,prefix);
	fprintf(fh,"X-OCCI-Attribute: %s.%s.id='%s'\r\n",prefix,nptr,(sptr->id?sptr->id:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.name='%s'\r\n",prefix,nptr,(sptr->name?sptr->name:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.description='%s'\r\n",prefix,nptr,(sptr->description?sptr->description:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.host='%s'\r\n",prefix,nptr,(sptr->host?sptr->host:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.user='%s'\r\n",prefix,nptr,(sptr->user?sptr->user:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.password='%s'\r\n",prefix,nptr,(sptr->password?sptr->password:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.subscription='%s'\r\n",prefix,nptr,(sptr->subscription?sptr->subscription:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.tenant='%s'\r\n",prefix,nptr,(sptr->tenant?sptr->tenant:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.version='%s'\r\n",prefix,nptr,(sptr->version?sptr->version:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.port='%u'\r\n",prefix,nptr,sptr->port);
	fprintf(fh,"X-OCCI-Attribute: %s.%s.state='%u'\r\n",prefix,nptr,sptr->state);
	return(0);

}

#endif	/* _paasconfig_cpaasconfig_c_ */
