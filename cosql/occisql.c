...
{
	char 	buffer[4096];
	sprintf(buffer,"MYSQL-QUERY: %s", msg);
	rest_log_message( buffer );
	return(0);
}
...
{
	char 	buffer[4096];
	sprintf(buffer,"MYSQL(%u,%s): TABLE(%s) FAILURE: %s \n",
	       	id,msg,tptr->name,mysql_error(tptr->handle));
	rest_log_message( buffer );
	return( result );
}
...
/*	-------------------------------		*/
private	int occi_mysql_query(int id, struct occi_table * tptr, char * xptr )
{
	int	status;
	debug_sql_query( tptr, xptr );
	occi_database_logwrite( xptr );
	if ((status = mysql_ping(tptr->handle)) != 0)
		return( occi_sql_failure( tptr, status, "ping", id ) );
	if ((status = mysql_query(tptr->handle, xptr)) != 0)
		return( occi_sql_failure( tptr, status, "query", id ) );
	else	return( status );
}
...
private	int	create_occi_sql_table( struct occi_table * tptr )
{
	...
	else 
	{
		...
		tptr->status = occi_mysql_query( 1,tptr, xptr );
		liberate( xptr );
...
private	int	create_occi_sql_database( struct occi_table * tptr )
{
	...
	else 
	{
		...
		if ((tptr->status = occi_mysql_query( 2,tptr, xptr )) != 0)
		{
			liberate( xptr );
			return(46);
		}
...
public	int	first_occi_sql_record( char * category,  struct occi_expression *expression )
{
	...
	occi_sql_lock();
	status = occi_mysql_query( 3,tptr, xptr );
	liberate( xptr );
	if ( status != 0)
	{
		occi_sql_unlock();
...
public	int	previous_occi_sql_record( char * category,  struct occi_expression *expression )
{
	...
	occi_sql_lock();
	status = occi_mysql_query( 4,tptr, xptr );
	liberate( xptr );
	if ( status != 0)
	{
		occi_sql_unlock();
...
public	int	insert_occi_sql_record( char * category,  struct occi_expression *expression )
{
	...
	occi_sql_lock();
	status = occi_mysql_query( 5,tptr, xptr );
	liberate( xptr );
	occi_sql_unlock();
	if ( status )
		return( 81 );
...
public	int	search_occi_sql_record( char * category,  struct occi_expression *expression )
{
	...
	occi_sql_lock();
	status = occi_mysql_query( 6,tptr, xptr );
	liberate( xptr );
	if ( status != 0 )
	{
		occi_sql_unlock();
...
public	int	collect_occi_sql_records( char * category,  struct occi_expression *expression )
{
	...
	if (!( expression ))
		...
		return( 27 );
			...
			_ORDER_BY_ORDERID);
	occi_sql_lock();
	status = occi_mysql_query( 7,tptr, xptr );
	liberate( xptr );
	if ( status )
	{
		occi_sql_unlock();
...
public	int	update_occi_sql_record( char * category,  struct occi_expression *expression )
{
	...
	occi_sql_lock();
	status = occi_mysql_query( 8,tptr, xptr );
	liberate( xptr );
	occi_sql_unlock();
	if ( status )
		return( 78 );
...
public	int	delete_occi_sql_record( char * category,  struct occi_expression *expression )
{
	...
	occi_sql_lock();
	status = occi_mysql_query( 9,tptr, xptr );
	liberate( xptr );
	occi_sql_unlock();
	if ( status )
		return( 78 );
...
public	int	next_occi_sql_record( char * category,  struct occi_expression *expression )
{
	...
	occi_sql_lock();
	status = occi_mysql_query( 10,tptr, xptr );
	liberate( xptr );
	if ( status != 0)
	{
		occi_sql_unlock();
...
public	int	last_occi_sql_record( char * category,  struct occi_expression *expression )
{
	...
	occi_sql_lock();
	status = occi_mysql_query( 11,tptr, xptr );
	liberate( xptr );
	if ( status != 0)
	{
		occi_sql_unlock();
...
public	int	start_sql_transaction(char * tablename)
{
	...
	{
		if (!(status = occi_mysql_query( 12,tptr, _START_TRANSACTION ) ))
			return( status );
		else	return( status );
	}
}
...
public	int	commit_sql_transaction(char * tablename)
{
	...
	{
		if (!(status = occi_mysql_query( 13,tptr, _COMMIT_TRANSACTION ) ))
			return( status );
		else	return( status );
	}
}
...
public	int	rollback_sql_transaction(char * tablename)
{
	...
	{
		if (!( status = occi_mysql_query( 14,tptr, _ROLLBACK_TRANSACTION ) ))
			return( status );
		else	return( status );
	}
}
