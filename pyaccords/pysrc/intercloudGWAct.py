...
from intercloudGWAction import *
from actionClass import *

def start(categStr):
	l=categStr.split(",")
	categoryAtr = CintercloudGW(l[0],l[1],l[2],l[3],l[4] )
	restCateg = intercloudGW_start(categoryAtr)
...
def stop(categStr):
	l=categStr.split(",")
	categoryAtr = CintercloudGW(l[0],l[1],l[2],l[3],l[4] )
	restCateg = intercloudGW_stop(categoryAtr)
	restResp = [str(restCateg.status),str(restCateg.message)]
	restResponse = ",".join(restResp)
	return restResponse
