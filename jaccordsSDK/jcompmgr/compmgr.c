...
int deleteJCategory(char categoryName[],int indice,int flag)
{
	...
	strcpy(pathf,JPATH);
 	...
 	sprintf(pathjcl,"%s/%s/%s/J%s.java",pathf,JACCORDS,JACCORDSS,categoryName);
	...
	sprintf(name1,"%s/%s/%s/J%sAct.java",pathf,JACCORDS,JACCORDSS,categoryName);
 	...
 	if((f=fopen(name1,"w"))==NULL)
  	...
  	else
  	{
		...
		fprintf(f,"\tstatic J%s objA;\n",categoryName);
		fprintf(f,"\tstatic J%s objB;\n",categoryName);
  	...
  	else
  	{
		fprintf(f,"/*----------------------------------------------------------------------------*/\n");
   		...
   		while(pelemaction)
   		{
     			fprintf(f,"\tpublic static J%s %s_%s(J%s obj, StringBuilder strMessage){\n",categoryName,categoryName,pelemaction->value,categoryName);
...
int generateCategoryJSourcefile(char *categoryName,listc categoryAtr,char pathf[])
{
 	...
 	sprintf(name1,"%s/%s/%s/J%s.java",pathf,JACCORDS,JACCORDSS,categoryName);
