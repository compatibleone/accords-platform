...
/*	---------------------------------------------------	*/
/*	 g e n e r a t e _ s e r v i c e _ b u i l d e r   	*/
/*	---------------------------------------------------	*/
...
public	int	generate_service_makefile( char * name, struct occi_category * cptr, char * prefix )
{
	...
	sprintf(buffer,"%s-Makefile.am",name);
	if (!( h =fopen( buffer, "w" ) ))
		return( 46 );
	else
	{
