...
#ifndef _eziconfig_c_
#define _eziconfig_c_
...
/*	--------------------------------------	*/
/*	l i b e r a t e _ e z i _ c o n f i g 	*/
/*	--------------------------------------	*/
public struct ezi_config * liberate_ezi_config(struct ezi_config * sptr)
{
	if ( sptr )
	{
		if ( sptr->id )
...
/*	--------------------------------	*/
/*	r e s e t _ e z i _ c o n f i g 	*/
/*	--------------------------------	*/
public struct ezi_config * reset_ezi_config(struct ezi_config * sptr)
{
	if ( sptr )
	{
		sptr->id = (char*) 0;
...
/*	--------------------------------------	*/
/*	a l l o c a t e _ e z i _ c o n f i g 	*/
/*	--------------------------------------	*/
public struct ezi_config * allocate_ezi_config()
{
	struct ezi_config * sptr;
	if (!( sptr = allocate( sizeof( struct ezi_config ) ) ))
		return( sptr );
	else	return( reset_ezi_config(sptr) );
}

/*	--------------------------------	*/
/*	x m l i n _ e z i _ c o n f i g 	*/
/*	--------------------------------	*/
public int xmlin_ezi_config(struct ezi_config * sptr,struct xml_element * eptr)
{
...
/*	----------------------------------------	*/
/*	r e s t _ o c c i _ e z i _ c o n f i g 	*/
/*	----------------------------------------	*/
public int rest_occi_ezi_config(FILE * fh,struct ezi_config * sptr,char * prefix, char * nptr)
{
	struct xml_element * wptr;
	if (!( sptr )) return(0);
...
#endif	/* _eziconfig_ceziconfig_c_ */
