	...
	struct nagios_service_group * pptr)
{
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.servicegroup_members=%u",optr->domain,optr->id,pptr->servicegroup_members);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	...
	struct rest_request * rptr, struct rest_response * aptr)
{
	...
	if (!( reqhost = rest_request_host( rptr ) ))
		return( rest_html_response( aptr, 400, "Bad Request" ) );
	else if (!( fptr = filter_nagios_service_group_info( optr, rptr, aptr ) ))
		return( rest_html_response( aptr, 400, "Bad Request" ) );
...
/*	------------------------------------	*/
private struct rest_response * delete_action_nagios_service_group(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_nagios_service_group_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_nagios_service_group_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "action_url",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_nagios_service_group)))
			return( optr );
