...
#ifndef _nagiosservicedependancy_c_
#define _nagiosservicedependancy_c_
...
/*	--------------------------------------------------------------------	*/
/*	l i b e r a t e _ n a g i o s _ s e r v i c e _ d e p e n d e n c y 	*/
/*	--------------------------------------------------------------------	*/
public struct nagios_service_dependency * liberate_nagios_service_dependency(struct nagios_service_dependency * sptr)
{
	if ( sptr )
	{
		if ( sptr->id )
		...
		if ( sptr->dependent_service_description )
			 sptr->dependent_service_description = liberate(sptr->dependent_service_description);
		if ( sptr->host_name )
		...
		if ( sptr->dependancy_period )
			 sptr->dependancy_period = liberate(sptr->dependancy_period);
		sptr = liberate( sptr );
	}
	return((struct nagios_service_dependency *) 0);

}

/*	--------------------------------------------------------------	*/
/*	r e s e t _ n a g i o s _ s e r v i c e _ d e p e n d e n c y 	*/
/*	--------------------------------------------------------------	*/
public struct nagios_service_dependency * reset_nagios_service_dependency(struct nagios_service_dependency * sptr)
{
	if ( sptr )
	{
		sptr->id = (char*) 0;
		...
		sptr->dependent_service_description = (char*) 0;
...
/*	--------------------------------------------------------------------	*/
/*	a l l o c a t e _ n a g i o s _ s e r v i c e _ d e p e n d e n c y 	*/
/*	--------------------------------------------------------------------	*/
public struct nagios_service_dependency * allocate_nagios_service_dependency()
{
	struct nagios_service_dependency * sptr;
	if (!( sptr = allocate( sizeof( struct nagios_service_dependency ) ) ))
		return( sptr );
	else	return( reset_nagios_service_dependency(sptr) );
}

/*	--------------------------------------------------------------	*/
/*	x m l i n _ n a g i o s _ s e r v i c e _ d e p e n d e n c y 	*/
/*	--------------------------------------------------------------	*/
public int xmlin_nagios_service_dependency(struct nagios_service_dependency * sptr,struct xml_element * eptr)
{
	...
	for ( wptr=eptr->first; wptr != (struct xml_element *) 0; wptr=wptr->next)
	{
		...
		else if (!( strcmp(wptr->name,"dependent_service_description") ))
		{
			if ( wptr->value ) { sptr->dependent_service_description = allocate_string(wptr->value); }
		}
		...
		else if (!( strcmp(wptr->name,"dependancy_period") ))
		{
			if ( wptr->value ) { sptr->dependancy_period = allocate_string(wptr->value); }
		}
...
/*	----------------------------------------------------------------------	*/
/*	r e s t _ o c c i _ n a g i o s _ s e r v i c e _ d e p e n d e n c y 	*/
/*	----------------------------------------------------------------------	*/
public int rest_occi_nagios_service_dependency(FILE * fh,struct nagios_service_dependency * sptr,char * prefix, char * nptr)
{
	struct xml_element * wptr;
	if (!( sptr )) return(0);
	...
	fprintf(fh,"X-OCCI-Attribute: %s.%s.dependent_service_description='%s'\r\n",prefix,nptr,(sptr->dependent_service_description?sptr->dependent_service_description:""));
	...
	fprintf(fh,"X-OCCI-Attribute: %s.%s.dependancy_period='%s'\r\n",prefix,nptr,(sptr->dependancy_period?sptr->dependancy_period:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.status='%u'\r\n",prefix,nptr,sptr->status);
	return(0);

}

#endif	/* _nagiosservicedependancy_cnagiosservicedependancy_c_ */
