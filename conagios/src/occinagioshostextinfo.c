	...
	struct rest_request * rptr, struct rest_response * aptr)
{
	...
	if (!( reqhost = rest_request_host( rptr ) ))
		return( rest_html_response( aptr, 400, "Bad Request" ) );
	else if (!( fptr = filter_nagios_host_ext_info_info( optr, rptr, aptr ) ))
		return( rest_html_response( aptr, 400, "Bad Request" ) );
...
/*	------------------------------------	*/
private struct rest_response * delete_action_nagios_host_ext_info(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_nagios_host_ext_info_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_nagios_host_ext_info_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "status",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_nagios_host_ext_info)))
			return( optr );
