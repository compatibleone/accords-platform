...
private	struct	az_config * resolve_az_configuration( char * sptr )
{
	...
	struct	occi_kind_node  * occi_first_az_config_node(char * category,char * token, char * value,int mode);
	struct	occi_kind_node  * occi_next_az_config_node(struct occi_kind_node  * nptr);
	for (	nptr = occi_first_az_config_node("windowazure_configuration","name",sptr,0);
