...
#ifndef _occischedule_c_
#define _occischedule_c_
...
public struct  occi_kind_node * occi_last_cords_schedule_node() { return( cords_schedule_last ); }
...
private void autoload_cords_schedule_nodes() {
	...
	if ((eptr = document_element(document,"cords_schedules")) != (struct xml_element *) 0) {
		for (vptr=eptr->first; vptr != (struct xml_element *) 0; vptr=vptr->next) {
			...
			if ((aptr = document_atribut( vptr, "response" )) != (struct xml_atribut *) 0)
				pptr->response = document_atribut_value(aptr);
			if ((aptr = document_atribut( vptr, "message" )) != (struct xml_atribut *) 0)
				pptr->message = document_atribut_string(aptr);
...
public  void autosave_cords_schedule_nodes() {
	...
	for ( nptr = cords_schedule_first;
		...
		fprintf(h,"%c",0x0022);
		fprintf(h," response=%c",0x0022);
		fprintf(h,"%u",pptr->response);
		fprintf(h,"%c",0x0022);
		fprintf(h," message=%c",0x0022);
		fprintf(h,"%s",(pptr->message?pptr->message:""));
		fprintf(h,"%c",0x0022);
	...
	struct occi_category * cptr,void * optr, char * nptr, char * vptr)
{
	...
	if (!( strncmp( nptr, prefix, strlen(prefix) ) )) {
		...
		if (!( strcmp( nptr, "response" ) ))
			pptr->response = atoi(vptr);
		if (!( strcmp( nptr, "message" ) ))
			pptr->message = allocate_string(vptr);
...
private int pass_cords_schedule_filter(
	...
	if (( fptr->response ) && ( pptr->response != fptr->response )) return(0);
	if (( fptr->message )
	&&  (strlen( fptr->message ) != 0)) {
		if (!( pptr->message ))
			return(0);
		else if ( strcmp(pptr->message,fptr->message) != 0)
			return(0);
		}
	...
	struct cords_schedule * pptr)
{
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.requested=%c%u%c",optr->domain,optr->id,0x0022,pptr->requested,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.expected=%c%u%c",optr->domain,optr->id,0x0022,pptr->expected,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.response=%c%u%c",optr->domain,optr->id,0x0022,pptr->response,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.message=%c%s%c",optr->domain,optr->id,0x0022,pptr->message,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_schedule(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_schedule_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_cords_schedule_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "priority",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "response",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "message",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "state",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_cords_schedule)))
			return( optr );
...
public struct rest_header *  cords_schedule_occi_headers(struct cords_schedule * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_schedule.response='%u'\r\n",sptr->response);
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_schedule.message='%s'\r\n",(sptr->message?sptr->message:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
