...
/*	------------------------------------	*/
/*	l i b e r a t e _ o a _ c o n f i g 	*/
/*	------------------------------------	*/
public struct oa_config * liberate_oa_config(struct oa_config * sptr)
{
	if ( sptr )
	{
		if ( sptr->id )
...
/*	------------------------------	*/
/*	r e s e t _ o a _ c o n f i g 	*/
/*	------------------------------	*/
public struct oa_config * reset_oa_config(struct oa_config * sptr)
{
	if ( sptr )
	{
		sptr->id = (char*) 0;
...
/*	------------------------------------	*/
/*	a l l o c a t e _ o a _ c o n f i g 	*/
/*	------------------------------------	*/
public struct oa_config * allocate_oa_config()
{
	struct oa_config * sptr;
	if (!( sptr = allocate( sizeof( struct oa_config ) ) ))
		return( sptr );
	else	return( reset_oa_config(sptr) );
}

/*	------------------------------	*/
/*	x m l i n _ o a _ c o n f i g 	*/
/*	------------------------------	*/
public int xmlin_oa_config(struct oa_config * sptr,struct xml_element * eptr)
{
	...
	for ( wptr=eptr->first; wptr != (struct xml_element *) 0; wptr=wptr->next)
	{
		...
		else if (!( strcmp(wptr->name,"authentication") ))
		{
			if ( wptr->value ) { sptr->authentication = allocate_string(wptr->value); }
		}
...
/*	--------------------------------------	*/
/*	r e s t _ o c c i _ o a _ c o n f i g 	*/
/*	--------------------------------------	*/
public int rest_occi_oa_config(FILE * fh,struct oa_config * sptr,char * prefix, char * nptr)
{
	struct xml_element * wptr;
	if (!( sptr )) return(0);
	...
	fprintf(fh,"X-OCCI-Attribute: %s.%s.authentication='%s'\r\n",prefix,nptr,(sptr->authentication?sptr->authentication:""));
