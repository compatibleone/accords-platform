...
{
	struct	oa_subscription * subscription;
...
/*	-------------------------------------------	*/
public	struct	rest_response * start_onapp(
		...
		void * vptr )
{
	...
	sprintf(buffer,"contract=%s/%s/%s\npublisher=%s\n",
		Configuration.identity,_CORDS_ONAPP,pptr->id,Configuration.publisher);
		...
		void * vptr )
{
	...
	else if ((oaptr = stop_onapp_provisioning( pptr )) == (struct oa_response *) 0)
		return(0);
	else
	{
		oaptr = oa_liberate_response( oaptr );

		sprintf(reference,"%s/%s/%s",Configuration.identity,_CORDS_ONAPP,pptr->id);
		if (!( rest_valid_string( pptr->price ) ))
			return( rest_html_response( aptr, 200, "OK" ) );
		else if ( occi_send_transaction( _CORDS_ONAPP, pptr->price, "action=stop", pptr->id, reference ) )
			return( rest_html_response( aptr, 200, "OK" ) );
