...
#ifndef _occiauthorise_c_
#define _occiauthorise_c_
...
public struct  occi_kind_node * occi_last_cords_authorization_node() { return( cords_authorization_last ); }
	...
	if ((eptr = document_element(document,"cords_authorizations")) != (struct xml_element *) 0) {
		for (vptr=eptr->first; vptr != (struct xml_element *) 0; vptr=vptr->next) {
			...
			if ((aptr = document_atribut( vptr, "from" )) != (struct xml_atribut *) 0)
				pptr->from = document_atribut_value(aptr);
			if ((aptr = document_atribut( vptr, "till" )) != (struct xml_atribut *) 0)
				pptr->till = document_atribut_value(aptr);
			if ((aptr = document_atribut( vptr, "ttl" )) != (struct xml_atribut *) 0)
				pptr->ttl = document_atribut_value(aptr);
...
public  void autosave_cords_authorization_nodes() {
	...
	for ( nptr = cords_authorization_first;
		...
		fprintf(h,"%u",pptr->from);
		fprintf(h,"%c",0x0022);
		fprintf(h," till=%c",0x0022);
		fprintf(h,"%u",pptr->till);
		fprintf(h,"%c",0x0022);
		fprintf(h," ttl=%c",0x0022);
		fprintf(h,"%u",pptr->ttl);
		fprintf(h,"%c",0x0022);
...
private int pass_cords_authorization_filter(
	...
	if (( fptr->from ) && ( pptr->from != fptr->from )) return(0);
	if (( fptr->till ) && ( pptr->till != fptr->till )) return(0);
	if (( fptr->ttl ) && ( pptr->ttl != fptr->ttl )) return(0);
	...
	struct cords_authorization * pptr)
{
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.from=%c%u%c",optr->domain,optr->id,0x0022,pptr->from,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.till=%c%u%c",optr->domain,optr->id,0x0022,pptr->till,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.ttl=%c%u%c",optr->domain,optr->id,0x0022,pptr->ttl,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
...
/*	--------------------------------------------------------------------------------	*/
private void	redirect_occi_cords_authorization_mt( struct rest_interface * iptr )
{
	iptr->get = occi_cords_authorization_get;
	iptr->post = occi_cords_authorization_post;
	iptr->put = occi_cords_authorization_put;
	iptr->delete = occi_cords_authorization_delete;
	iptr->head = occi_cords_authorization_head;
	return;
}
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_authorization(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_authorization_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_cords_authorization_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		redirect_occi_cords_authorization_mt(optr->interface);
		...
		if (!( optr = occi_add_attribute(optr, "from",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "till",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "ttl",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "state",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_cords_authorization)))
			return( optr );
