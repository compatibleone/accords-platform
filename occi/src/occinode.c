...
public struct  occi_kind_node * occi_last_cords_node_node() { return( cords_node_last ); }
...
/*	--------------------------------------------------------------------------------	*/
private void	redirect_occi_cords_node_mt( struct rest_interface * iptr )
{
	iptr->get = occi_cords_node_get;
	iptr->post = occi_cords_node_post;
	iptr->put = occi_cords_node_put;
	iptr->delete = occi_cords_node_delete;
	iptr->head = occi_cords_node_head;
	return;
}
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_node(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_node_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_cords_node_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		redirect_occi_cords_node_mt(optr->interface);
		...
		if (!( optr = occi_add_attribute(optr, "state",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_cords_node)))
			return( optr );
...
/*	----------------------------------------------	*/
/*	c o r d s _ n o d e _ o c c i _ h e a d e r s 	*/
/*	----------------------------------------------	*/
public struct rest_header *  cords_node_occi_headers(struct cords_node * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_node.access='%s'\r\n",(sptr->access?sptr->access:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_node.scope='%s'\r\n",(sptr->scope?sptr->scope:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_node.firewall='%s'\r\n",(sptr->firewall?sptr->firewall:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_node.operator='%s'\r\n",(sptr->operator?sptr->operator:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
