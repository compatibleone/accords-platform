...
/*	--------------------------------------------------------------------	*/
struct gw * allocate_gw();
struct gw * liberate_gw(struct gw * optr);
private pthread_mutex_t list_gw_control=PTHREAD_MUTEX_INITIALIZER;
...
private struct occi_kind_node * drop_gw_node(struct occi_kind_node * nptr) {
	...
	nptr = ll_drop_gw_node( nptr );
	pthread_mutex_unlock( &list_gw_control );
	return(nptr);
}
...
/*	------------------------------------------------------------------------------------------	*/
private char*autosave_gw_name="gw.xml";
private void autoload_gw_nodes() {
	char * fn=autosave_gw_name;	struct occi_kind_node * nptr;
	struct gw * pptr;
	...
	if (!( document = document_parse_file(fn)))
		return;
	if ((eptr = document_element(document,"gws")) != (struct xml_element *) 0) {
		for (vptr=eptr->first; vptr != (struct xml_element *) 0; vptr=vptr->next) {
			if (!( vptr->name )) continue;
			else if ( strcmp( vptr->name, "gw" ) ) continue;
			else if (!( nptr = add_gw_node(0))) break;
			...
			if ((aptr = document_atribut( vptr, "ethername" )) != (struct xml_atribut *) 0)
				pptr->ethername = document_atribut_string(aptr);
			...
			if ((aptr = document_atribut( vptr, "provider_type" )) != (struct xml_atribut *) 0)
				pptr->provider_type = document_atribut_string(aptr);
			if ((aptr = document_atribut( vptr, "provider_platform" )) != (struct xml_atribut *) 0)
				pptr->provider_platform = document_atribut_string(aptr);
...
/*	------------------------------------------------------------------------------------------	*/
public  void set_autosave_gw_name(char * fn) {
	autosave_gw_name = fn;	return;
}
public  void autosave_gw_nodes() {
	char * fn=autosave_gw_name;	struct occi_kind_node * nptr;
	struct gw * pptr;
	FILE * h;
	pthread_mutex_lock( &list_gw_control );
	...
	for ( nptr = gw_first;
		...
		fprintf(h,"%c",0x0022);
		fprintf(h," ethername=%c",0x0022);
		fprintf(h,"%s",(pptr->ethername?pptr->ethername:""));
		fprintf(h,"%c",0x0022);
		...
		fprintf(h,"%c",0x0022);
		fprintf(h," provider_type=%c",0x0022);
		fprintf(h,"%s",(pptr->provider_type?pptr->provider_type:""));
		fprintf(h,"%c",0x0022);
		fprintf(h," provider_platform=%c",0x0022);
		fprintf(h,"%s",(pptr->provider_platform?pptr->provider_platform:""));
		fprintf(h,"%c",0x0022);
...
struct occi_category * cptr,void * optr, char * nptr, char * vptr)
{
	...
	if (!( strncmp( nptr, prefix, strlen(prefix) ) )) {
		...
		if (!( strcmp( nptr, "ethername" ) ))
			pptr->ethername = allocate_string(vptr);
		...
		if (!( strcmp( nptr, "provider_type" ) ))
			pptr->provider_type = allocate_string(vptr);
		if (!( strcmp( nptr, "provider_platform" ) ))
			pptr->provider_platform = allocate_string(vptr);
...
/*	--------------------------------------------------	*/
private struct gw * filter_gw_info(
...
/*	--------------------------------------------------	*/
private int pass_gw_filter(
	struct gw * pptr,struct gw * fptr) {
	if (( fptr->id )
	&&  (strlen( fptr->id ) != 0)) {
		if (!( pptr->id ))
			return(0);
...
/*	------------------------------------------------------------------------------------------	*/
private struct rest_response * gw_post_link(
	struct occi_category * optr, struct rest_client * cptr,
	struct rest_request * rptr, struct rest_response * aptr,char * id)
{
	struct rest_header * hptr;
...
/*	------------------------------------------------------------------------------------------	*/
private struct rest_response * gw_post_item(
	struct occi_category * optr, struct rest_client * cptr,
	struct rest_request * rptr, struct rest_response * aptr)
{
	struct rest_header * hptr;
...
/*	----------------------------------------------------------------------------------------	*/
private struct rest_response * gw_put_item(
	struct occi_category * optr, struct rest_client * cptr,
	struct rest_request * rptr, struct rest_response * aptr,char * id)
{
	struct rest_header * hptr;
...
/*	------------------------------------------------------------------------------------------	*/
private struct rest_response * gw_head_item(
	struct occi_category * optr, struct rest_client * cptr,
	struct rest_request * rptr, struct rest_response * aptr,char * id)
{
	struct rest_header * hptr;
...
/*	----------------------------------------------------------------------------------------------	*/
private struct rest_response * gw_delete_item(
	struct occi_category * optr, struct rest_client * cptr,
	struct rest_request * rptr, struct rest_response * aptr, char * id)
{
	struct rest_header * hptr;
...
/*	----------------------------------------------------------------------------------------	*/
private struct rest_response * gw_get_list(
	struct occi_category * optr, struct rest_client * cptr,
	struct rest_request * rptr, struct rest_response * aptr)
{
	struct rest_header * hptr;
	...
	if (!( reqhost = rest_request_host( rptr ) ))
		return( rest_html_response( aptr, 400, "Bad Request" ) );
	else if (!( fptr = filter_gw_info( optr, rptr, aptr ) ))
		return( rest_html_response( aptr, 400, "Bad Request" ) );
	for ( sptr = gw_first;
...
/*	--------------------------------------------------------------------------------------------	*/
private struct rest_response * gw_delete_all(
	struct occi_category * optr, struct rest_client * cptr,
	struct rest_request * rptr, struct rest_response * aptr)
{
	struct rest_header * hptr;
	...
	struct gw * fptr;
	iptr = optr->callback;
	if (!( fptr = filter_gw_info( optr, rptr, aptr ) ))
		return( rest_html_response( aptr, 400, "Bad Request" ) );
	nptr=gw_first;
	while (nptr != (struct occi_kind_node *) 0) {
		if ((!( pptr = nptr->contents ))
		||  (!( pass_gw_filter( pptr, fptr ) ))) {
			nptr = nptr->next;
			continue;
			}
		else	{
...
/*	------------------------------------------------------------------------------	*/
private struct rest_response * occi_gw_get(void * vptr, struct rest_client * cptr, struct rest_request * rptr)
{
	struct rest_response * aptr;
	struct rest_header   * hptr;
	struct occi_category * optr;
	...
	if(!(aptr = rest_allocate_response( cptr )))
		...
		return( gw_get_list( optr, cptr, rptr, aptr ) );
	else if (!(strncmp( rptr->object, optr->location, strlen(optr->location) ) ))
		return( gw_get_item( optr, cptr, rptr, aptr,rptr->object+strlen(optr->location) ) );
	else	return( rest_html_response( aptr, 400, "Bad Request") );
}
...
/*	--------------------------------------------------------------------------------	*/
private struct rest_response * occi_gw_head(void * vptr, struct rest_client * cptr, struct rest_request * rptr)
{
	struct rest_response * aptr;
	struct rest_header   * hptr;
	struct occi_category * optr;
	...
	if(!(aptr = rest_allocate_response( cptr )))
		...
		return( gw_head_item( optr, cptr, rptr, aptr,rptr->object+strlen(optr->location) ) );
	else	return( rest_html_response( aptr, 400, "Bad Request") );
}
...
/*	--------------------------------------------------------------------------------	*/
private struct rest_response * occi_gw_post(void * vptr, struct rest_client * cptr, struct rest_request * rptr)
{
	struct rest_response * aptr;
	struct rest_header   * hptr;
	struct occi_category * optr;
	...
	if(!(aptr = rest_allocate_response( cptr )))
		...
		return( gw_post_item( optr, cptr, rptr, aptr ) );
	else if ( strncmp( rptr->object, optr->location,strlen(optr->location)) != 0)
		return( rest_html_response( aptr, 400, "Bad Request") );
	else if (!( rptr->parameters ))
		return( rest_html_response( aptr, 400, "Bad Request") );
	else if (!( strncmp( rptr->parameters, "action=", strlen("action=")) ))
		return( gw_post_action( optr, cptr, rptr, aptr,rptr->object+strlen(optr->location) ) );
	else if (!( strncmp( rptr->parameters, "mixin=", strlen("mixin=")) ))
		return( gw_post_mixin( optr, cptr, rptr, aptr,rptr->object+strlen(optr->location) ) );
	else if (!( strncmp( rptr->parameters, "link=", strlen("link=")) ))
		return( gw_post_link( optr, cptr, rptr, aptr,rptr->object+strlen(optr->location) ) );
	else	return( rest_html_response( aptr, 400, "Bad Request") );
}
...
/*	------------------------------------------------------------------------------	*/
private struct rest_response * occi_gw_put(void * vptr, struct rest_client * cptr, struct rest_request * rptr)
{
	struct rest_response * aptr;
	struct rest_header   * hptr;
	struct occi_category * optr;
	...
	if(!(aptr = rest_allocate_response( cptr )))
		...
		return( gw_put_item( optr, cptr, rptr, aptr,rptr->object+strlen(optr->location) ) );
	else	return( rest_html_response( aptr, 400, "Bad Request") );
}
...
/*	------------------------------------------------------------------------------------	*/
private struct rest_response * occi_gw_delete(void * vptr, struct rest_client * cptr, struct rest_request * rptr)
{
	struct rest_response * aptr;
	struct rest_header   * hptr;
	struct occi_category * optr;
	...
	if(!(aptr = rest_allocate_response( cptr )))
		...
		return( gw_delete_all( optr, cptr, rptr, aptr ) );
	else if (!(strncmp( rptr->object, optr->location, strlen(optr->location) ) ))
		return( gw_delete_item( optr, cptr, rptr, aptr,rptr->object+strlen(optr->location) ) );
	else	return( rest_html_response( aptr, 400, "Bad Request") );
}
...
private void	redirect_occi_gw_mt( struct rest_interface * iptr )
{
	...
	iptr->post = occi_gw_post;
	iptr->put = occi_gw_put;
	iptr->delete = occi_gw_delete;
	iptr->head = occi_gw_head;
	return;
}
...
public struct occi_category * occi_cords_gw_builder(char * a,char * b) {
	...
	char * f="CompatibleOne OCCI resource gw";
	struct occi_category * optr;
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
	else {
		redirect_occi_gw_mt(optr->interface);
		...
		if (!( optr = occi_add_attribute(optr, "privateaddr",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "ethername",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "intercloudGW",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "contract",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "provider_type",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "provider_platform",0,0) ))
			return(optr);
...
public struct rest_header *  gw_occi_headers(struct gw * sptr)
{
	...
	if (!( hptr->name = allocate_string("Category")))
		return(first);
	sprintf(buffer,"gw; scheme='http://scheme.compatibleone.fr/scheme/compatible#'; class='kind';\r\n");
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.publicaddr='%s'\r\n",(sptr->publicaddr?sptr->publicaddr:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.privateaddr='%s'\r\n",(sptr->privateaddr?sptr->privateaddr:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.ethername='%s'\r\n",(sptr->ethername?sptr->ethername:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.intercloudGW='%s'\r\n",(sptr->intercloudGW?sptr->intercloudGW:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.contract='%s'\r\n",(sptr->contract?sptr->contract:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.provider_type='%s'\r\n",(sptr->provider_type?sptr->provider_type:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.provider_platform='%s'\r\n",(sptr->provider_platform?sptr->provider_platform:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.connection='%s'\r\n",(sptr->connection?sptr->connection:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.account='%s'\r\n",(sptr->account?sptr->account:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.gw.state='%s'\r\n",(sptr->state?sptr->state:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	return(first);
}
