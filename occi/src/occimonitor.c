...
#ifndef _occimonitor_c_
#define _occimonitor_c_
...
public struct  occi_kind_node * occi_last_cords_monitor_node() { return( cords_monitor_last ); }
	...
	struct cords_monitor * pptr)
{
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.report=%c%s%c",optr->domain,optr->id,0x0022,pptr->report,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.controls=%c%u%c",optr->domain,optr->id,0x0022,pptr->controls,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_monitor(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_monitor_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_cords_monitor_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "state",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_cords_monitor)))
			return( optr );
...
public struct rest_header *  cords_monitor_occi_headers(struct cords_monitor * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_monitor.connection='%s'\r\n",(sptr->connection?sptr->connection:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
