...
#ifndef _occionconfig_c_
#define _occionconfig_c_
...
/*	--------------------------------------------------------------------------------	*/
private void	redirect_occi_on_config_mt( struct rest_interface * iptr )
{
	iptr->get = occi_on_config_get;
	iptr->post = occi_on_config_post;
	iptr->put = occi_on_config_put;
	iptr->delete = occi_on_config_delete;
	iptr->head = occi_on_config_head;
	return;
}
...
/*	------------------------------------	*/
private struct rest_response * delete_action_on_config(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_on_config_delete(optr,cptr,rptr));
}

/*	------------------------------	*/
/*	c r u d   p u t   a c t i o n 	*/
/*	------------------------------	*/
private struct rest_response * put_action_on_config(struct occi_category * optr, 
struct rest_client * cptr,  
struct rest_request * rptr,  
struct rest_response * aptr,  
void * vptr )
{
	aptr = liberate_rest_response( aptr );
	return( occi_on_config_put(optr,cptr,rptr));
}
...
public struct occi_category * occi_on_config_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		redirect_occi_on_config_mt(optr->interface);
		...
		if (!( optr = occi_add_attribute(optr, "current",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_on_config)))
			return( optr );
		if (!( optr = occi_add_action( optr,"PUT","",put_action_on_config)))
			return( optr );
