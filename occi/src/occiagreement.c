...
#ifndef _occiagreement_c_
#define _occiagreement_c_
...
public struct  occi_kind_node * occi_last_cords_agreement_node() { return( cords_agreement_last ); }
...
private void autoload_cords_agreement_nodes() {
	...
	if ((eptr = document_element(document,"cords_agreements")) != (struct xml_element *) 0) {
		for (vptr=eptr->first; vptr != (struct xml_element *) 0; vptr=vptr->next) {
			...
			if ((aptr = document_atribut( vptr, "termss" )) != (struct xml_atribut *) 0)
				pptr->termss = document_atribut_value(aptr);
...
public  void autosave_cords_agreement_nodes() {
	...
	for ( nptr = cords_agreement_first;
		...
		fprintf(h,"%c",0x0022);
		fprintf(h," termss=%c",0x0022);
		fprintf(h,"%u",pptr->termss);
		fprintf(h,"%c",0x0022);
	...
	struct occi_category * cptr,void * optr, char * nptr, char * vptr)
{
	...
	if (!( strncmp( nptr, prefix, strlen(prefix) ) )) {
		...
		if (!( strcmp( nptr, "termss" ) ))
			pptr->termss = atoi(vptr);
...
private int pass_cords_agreement_filter(
	...
	if (( fptr->termss ) && ( pptr->termss != fptr->termss )) return(0);
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_agreement(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_agreement_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_cords_agreement_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "templatename",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "termss",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "state",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_cords_agreement)))
			return( optr );
...
public struct rest_header *  cords_agreement_occi_headers(struct cords_agreement * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_agreement.termss='%u'\r\n",sptr->termss);
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
