...
#ifndef _occiagency_c_
#define _occiagency_c_
...
public struct  occi_kind_node * occi_last_cords_agency_node() { return( cords_agency_last ); }
...
private void autoload_cords_agency_nodes() {
	...
	if ((eptr = document_element(document,"cords_agencys")) != (struct xml_element *) 0) {
		for (vptr=eptr->first; vptr != (struct xml_element *) 0; vptr=vptr->next) {
			...
			if ((aptr = document_atribut( vptr, "servicename" )) != (struct xml_atribut *) 0)
				pptr->servicename = document_atribut_string(aptr);
			...
			if ((aptr = document_atribut( vptr, "publisher" )) != (struct xml_atribut *) 0)
				pptr->publisher = document_atribut_string(aptr);
			...
			if ((aptr = document_atribut( vptr, "start" )) != (struct xml_atribut *) 0)
				pptr->start = document_atribut_value(aptr);
...
public  void autosave_cords_agency_nodes() {
	...
	for ( nptr = cords_agency_first;
		...
		fprintf(h,"%c",0x0022);
		fprintf(h," servicename=%c",0x0022);
		fprintf(h,"%s",(pptr->servicename?pptr->servicename:""));
		fprintf(h,"%c",0x0022);
		...
		fprintf(h,"%c",0x0022);
		fprintf(h," publisher=%c",0x0022);
		fprintf(h,"%s",(pptr->publisher?pptr->publisher:""));
		fprintf(h,"%c",0x0022);
	...
	struct occi_category * cptr,void * optr, char * nptr, char * vptr)
{
	...
	if (!( strncmp( nptr, prefix, strlen(prefix) ) )) {
		nptr += strlen(prefix);
		if (!( strcmp( nptr, "servicename" ) ))
			pptr->servicename = allocate_string(vptr);
		...
		if (!( strcmp( nptr, "publisher" ) ))
			pptr->publisher = allocate_string(vptr);
		if (!( strcmp( nptr, "user" ) ))
...
private int pass_cords_agency_filter(
	...
	if (( fptr->servicename )
	&&  (strlen( fptr->servicename ) != 0)) {
		if (!( pptr->servicename ))
			return(0);
		else if ( strcmp(pptr->servicename,fptr->servicename) != 0)
			return(0);
		}
	if (( fptr->description )
	...
	if (( fptr->publisher )
	&&  (strlen( fptr->publisher ) != 0)) {
		if (!( pptr->publisher ))
			return(0);
		else if ( strcmp(pptr->publisher,fptr->publisher) != 0)
			return(0);
		}
	if (( fptr->user )
	&&  (strlen( fptr->user ) != 0)) {
	...
	if (( fptr->start ) && ( pptr->start != fptr->start )) return(0);
	...
	struct cords_agency * pptr)
{
	...
	sprintf(cptr->buffer,"%s.%s.servicename=%c%s%c",optr->domain,optr->id,0x0022,pptr->servicename,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.publisher=%c%s%c",optr->domain,optr->id,0x0022,pptr->publisher,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.start=%c%u%c",optr->domain,optr->id,0x0022,pptr->start,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_agency(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_agency_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_cords_agency_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "domain",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "publisher",0,0) ))
			return(optr);
		...
		if (!( optr = occi_add_attribute(optr, "state",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_cords_agency)))
			return( optr );
