...
#ifndef _occiterms_c_
#define _occiterms_c_
...
public struct  occi_kind_node * occi_last_cords_terms_node() { return( cords_terms_last ); }
	...
	struct cords_terms * pptr)
{
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.terms=%c%u%c",optr->domain,optr->id,0x0022,pptr->terms,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_terms(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_terms_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_cords_terms_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "state",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_cords_terms)))
			return( optr );
...
public struct rest_header *  cords_terms_occi_headers(struct cords_terms * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_terms.state='%u'\r\n",sptr->state);
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	return(first);
