...
public struct  occi_kind_node * occi_last_cords_file_node() { return( cords_file_last ); }
	...
	struct cords_file * pptr)
{
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.length=%c%u%c",optr->domain,optr->id,0x0022,pptr->length,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.permissions=%c%s%c",optr->domain,optr->id,0x0022,pptr->permissions,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_file(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_file_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_cords_file_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "permissions",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_cords_file)))
			return( optr );
