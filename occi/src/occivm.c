...
public struct  occi_kind_node * occi_last_cords_vm_node() { return( cords_vm_last ); }
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_vm(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_vm_delete(optr,cptr,rptr));
}
...
public struct occi_category * occi_cords_vm_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "state",0,0) ))
			return(optr);
		if (!( optr = occi_add_action( optr,"DELETE","",delete_action_cords_vm)))
			return( optr );
...
public struct rest_header *  cords_vm_occi_headers(struct cords_vm * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_vm.state='%u'\r\n",sptr->state);
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	return(first);
