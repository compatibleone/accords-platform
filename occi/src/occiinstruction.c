...
#ifndef _occiinstruction_c_
#define _occiinstruction_c_
...
public struct  occi_kind_node * occi_last_cords_instruction_node() { return( cords_instruction_last ); }
	...
	struct cords_instruction * pptr)
{
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.method=%c%s%c",optr->domain,optr->id,0x0022,pptr->method,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.type=%c%s%c",optr->domain,optr->id,0x0022,pptr->type,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.symbol=%c%s%c",optr->domain,optr->id,0x0022,pptr->symbol,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
...
/*	------------------------------------	*/
private struct rest_response * delete_action_cords_instruction(struct occi_category * optr, 
...
void * vptr )
{
	...
	return( occi_cords_instruction_delete(optr,cptr,rptr));
}
...
public struct rest_header *  cords_instruction_occi_headers(struct cords_instruction * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_instruction.nature='%s'\r\n",(sptr->nature?sptr->nature:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
