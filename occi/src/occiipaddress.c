...
/*	--------------------------------------------------------------------	*/
private struct rest_interface occi_cords_ipaddress_mt = {
	(void*) 0,
	(void*) 0,
	(void*) 0,
	occi_cords_ipaddress_get,
	occi_cords_ipaddress_post,
	occi_cords_ipaddress_put,
	occi_cords_ipaddress_delete,
	occi_cords_ipaddress_head,
	(void*) 0
	};
...
public struct occi_category * occi_cords_ipaddress_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		optr->interface = &occi_cords_ipaddress_mt;
...
public struct rest_header *  cords_ipaddress_occi_headers(struct cords_ipaddress * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_ipaddress.timestamp='%u'\r\n",sptr->timestamp);
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
