...
private void autoload_cords_invoice_nodes() {
	...
	if ((eptr = document_element(document,"cords_invoices")) != (struct xml_element *) 0) {
		for (vptr=eptr->first; vptr != (struct xml_element *) 0; vptr=vptr->next) {
			...
			if ((aptr = document_atribut( vptr, "lines" )) != (struct xml_atribut *) 0)
				pptr->lines = document_atribut_value(aptr);
...
public  void autosave_cords_invoice_nodes() {
	...
	for ( nptr = cords_invoice_first;
		...
		fprintf(h,"%c",0x0022);
		fprintf(h," lines=%c",0x0022);
		fprintf(h,"%u",pptr->lines);
		fprintf(h,"%c",0x0022);
	...
	struct occi_category * cptr,void * optr, char * nptr, char * vptr)
{
	...
	if (!( strncmp( nptr, prefix, strlen(prefix) ) )) {
		...
		if (!( strcmp( nptr, "lines" ) ))
			pptr->lines = atoi(vptr);
...
private int pass_cords_invoice_filter(
	...
	if (( fptr->lines ) && ( pptr->lines != fptr->lines )) return(0);
	...
	struct cords_invoice * pptr)
{
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.total=%s",optr->domain,optr->id,pptr->total);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.lines=%u",optr->domain,optr->id,pptr->lines);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	...
	struct rest_request * rptr, struct rest_response * aptr)
{
	...
	if (!( reqhost = rest_request_host( rptr ) ))
		return( rest_html_response( aptr, 400, "Bad Request" ) );
	else if (!( fptr = filter_cords_invoice_info( optr, rptr, aptr ) ))
		return( rest_html_response( aptr, 400, "Bad Request" ) );
...
/*	--------------------------------------------------------------------	*/
private struct rest_interface occi_cords_invoice_mt = {
	(void*) 0,
	(void*) 0,
	(void*) 0,
	occi_cords_invoice_get,
	occi_cords_invoice_post,
	occi_cords_invoice_put,
	occi_cords_invoice_delete,
	occi_cords_invoice_head,
	(void*) 0
	};
...
public struct occi_category * occi_cords_invoice_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		optr->interface = &occi_cords_invoice_mt;
		...
		if (!( optr = occi_add_attribute(optr, "rate",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "lines",0,0) ))
			return(optr);
...
public struct rest_header *  cords_invoice_occi_headers(struct cords_invoice * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_invoice.authorisation='%s'\r\n",(sptr->authorisation?sptr->authorisation:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_invoice.lines='%u'\r\n",sptr->lines);
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.cords_invoice.status='%u'\r\n",sptr->status);
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	return(first);
