...
private void autoload_easiclouds_model_nodes() {
	...
	if ((eptr = document_element(document,"easiclouds_models")) != (struct xml_element *) 0) {
		for (vptr=eptr->first; vptr != (struct xml_element *) 0; vptr=vptr->next) {
			...
			if ((aptr = document_atribut( vptr, "inputs" )) != (struct xml_atribut *) 0)
				pptr->inputs = document_atribut_string(aptr);
			if ((aptr = document_atribut( vptr, "outputs" )) != (struct xml_atribut *) 0)
				pptr->outputs = document_atribut_string(aptr);
			if ((aptr = document_atribut( vptr, "instances" )) != (struct xml_atribut *) 0)
				pptr->instances = document_atribut_value(aptr);
...
public  void autosave_easiclouds_model_nodes() {
	...
	for ( nptr = easiclouds_model_first;
		...
		fprintf(h,"%c",0x0022);
		fprintf(h," inputs=%c",0x0022);
		fprintf(h,"%s",(pptr->inputs?pptr->inputs:""));
		fprintf(h,"%c",0x0022);
		fprintf(h," outputs=%c",0x0022);
		fprintf(h,"%s",(pptr->outputs?pptr->outputs:""));
		fprintf(h,"%c",0x0022);
		fprintf(h," instances=%c",0x0022);
		fprintf(h,"%u",pptr->instances);
		fprintf(h,"%c",0x0022);
	...
	struct occi_category * cptr,void * optr, char * nptr, char * vptr)
{
	...
	if (!( strncmp( nptr, prefix, strlen(prefix) ) )) {
		...
		if (!( strcmp( nptr, "inputs" ) ))
			pptr->inputs = allocate_string(vptr);
		if (!( strcmp( nptr, "outputs" ) ))
			pptr->outputs = allocate_string(vptr);
		if (!( strcmp( nptr, "instances" ) ))
			pptr->instances = atoi(vptr);
...
private int pass_easiclouds_model_filter(
	...
	if (( fptr->inputs )
	&&  (strlen( fptr->inputs ) != 0)) {
		if (!( pptr->inputs ))
			return(0);
		else if ( strcmp(pptr->inputs,fptr->inputs) != 0)
			return(0);
		}
	if (( fptr->outputs )
	&&  (strlen( fptr->outputs ) != 0)) {
		if (!( pptr->outputs ))
			return(0);
		else if ( strcmp(pptr->outputs,fptr->outputs) != 0)
			return(0);
		}
	if (( fptr->instances ) && ( pptr->instances != fptr->instances )) return(0);
	...
	struct easiclouds_model * pptr)
{
	...
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.inputs=%c%s%c",optr->domain,optr->id,0x0022,pptr->inputs,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.outputs=%c%s%c",optr->domain,optr->id,0x0022,pptr->outputs,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
	sprintf(cptr->buffer,"%s.%s.instances=%c%u%c",optr->domain,optr->id,0x0022,pptr->instances,0x0022);
	if (!( hptr = rest_response_header( aptr, "X-OCCI-Attribute",cptr->buffer) ))
		return( rest_html_response( aptr, 500, "Server Failure" ) );
...
public struct occi_category * occi_easiclouds_model_builder(char * a,char * b) {
	...
	if (!( optr = occi_create_category(a,b,c,d,e,f) )) { return(optr); }
		...
		if (!( optr = occi_add_attribute(optr, "number",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "inputs",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "outputs",0,0) ))
			return(optr);
		if (!( optr = occi_add_attribute(optr, "instances",0,0) ))
			return(optr);
...
public struct rest_header *  easiclouds_model_occi_headers(struct easiclouds_model * sptr)
{
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.easiclouds_model.inputs='%s'\r\n",(sptr->inputs?sptr->inputs:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.easiclouds_model.outputs='%s'\r\n",(sptr->outputs?sptr->outputs:""));
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
	...
	if (!( hptr->name = allocate_string("X-OCCI-Attribute")))
		return(first);
	sprintf(buffer,"occi.easiclouds_model.instances='%u'\r\n",sptr->instances);
	if (!( hptr->value = allocate_string(buffer)))
		return(first);
	if (!( hptr = allocate_rest_header()))
		return(first);
