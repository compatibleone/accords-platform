/* -------------------------------------------------------------------- */
/*  ACCORDS PLATFORM                                                    */
/*  (C) 2011 by Iain James Marshall (Prologue) <ijm667@hotmail.com>     */
/* -------------------------------------------------------------------- */
/* Licensed under the Apache License, Version 2.0 (the "License"); 	*/
/* you may not use this file except in compliance with the License. 	*/
/* You may obtain a copy of the License at 				*/
/*  									*/
/*  http://www.apache.org/licenses/LICENSE-2.0 				*/
/*  									*/
/* Unless required by applicable law or agreed to in writing, software 	*/
/* distributed under the License is distributed on an "AS IS" BASIS, 	*/
/* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or 	*/
/* implied. 								*/
/* See the License for the specific language governing permissions and 	*/
/* limitations under the License. 					*/
/* -------------------------------------------------------------------- */

/* STRUKT WARNING : this file has been generated and should not be modified by hand */
#ifndef _easiclouds_model_c_
#define _easiclouds_model_c_

#include "element.h"

#include "easiclouds_model.h"

/*	--------------------------------------------------	*/
/*	l i b e r a t e _ e a s i c l o u d s _ m o d e l 	*/
/*	--------------------------------------------------	*/
public struct easiclouds_model * liberate_easiclouds_model(struct easiclouds_model * sptr)
{
	if ( sptr )
	{
		if ( sptr->id )
			 sptr->id = liberate(sptr->id);
		if ( sptr->name )
			 sptr->name = liberate(sptr->name);
		if ( sptr->type )
			 sptr->type = liberate(sptr->type);
		if ( sptr->size )
			 sptr->size = liberate(sptr->size);
		if ( sptr->number )
			 sptr->number = liberate(sptr->number);
		if ( sptr->inputs )
			 sptr->inputs = liberate(sptr->inputs);
		if ( sptr->outputs )
			 sptr->outputs = liberate(sptr->outputs);
		sptr = liberate( sptr );
	}
	return((struct easiclouds_model *) 0);

}

/*	--------------------------------------------	*/
/*	r e s e t _ e a s i c l o u d s _ m o d e l 	*/
/*	--------------------------------------------	*/
public struct easiclouds_model * reset_easiclouds_model(struct easiclouds_model * sptr)
{
	if ( sptr )
	{
		sptr->id = (char*) 0;
		sptr->name = (char*) 0;
		sptr->type = (char*) 0;
		sptr->size = (char*) 0;
		sptr->number = (char*) 0;
		sptr->inputs = (char*) 0;
		sptr->outputs = (char*) 0;
		sptr->instances =  0;
		sptr->state =  0;
	}
	return(sptr);

}

/*	--------------------------------------------------	*/
/*	a l l o c a t e _ e a s i c l o u d s _ m o d e l 	*/
/*	--------------------------------------------------	*/
public struct easiclouds_model * allocate_easiclouds_model()
{
	struct easiclouds_model * sptr;
	if (!( sptr = allocate( sizeof( struct easiclouds_model ) ) ))
		return( sptr );
	else	return( reset_easiclouds_model(sptr) );
}

/*	--------------------------------------------	*/
/*	x m l i n _ e a s i c l o u d s _ m o d e l 	*/
/*	--------------------------------------------	*/
public int xmlin_easiclouds_model(struct easiclouds_model * sptr,struct xml_element * eptr)
{
	struct xml_element * wptr;
	if (!( eptr )) return(0);
	if (!( sptr )) return(0);
	for ( wptr=eptr->first; wptr != (struct xml_element *) 0; wptr=wptr->next)
	{
		if (!( strcmp(wptr->name,"id") ))
		{
			if ( wptr->value ) { sptr->id = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"name") ))
		{
			if ( wptr->value ) { sptr->name = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"type") ))
		{
			if ( wptr->value ) { sptr->type = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"size") ))
		{
			if ( wptr->value ) { sptr->size = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"number") ))
		{
			if ( wptr->value ) { sptr->number = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"inputs") ))
		{
			if ( wptr->value ) { sptr->inputs = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"outputs") ))
		{
			if ( wptr->value ) { sptr->outputs = allocate_string(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"instances") ))
		{
			if ( wptr->value ) { sptr->instances = atoi(wptr->value); }
		}
		else if (!( strcmp(wptr->name,"state") ))
		{
			if ( wptr->value ) { sptr->state = atoi(wptr->value); }
		}
	}
	return(0);

}

/*	----------------------------------------------------	*/
/*	r e s t _ o c c i _ e a s i c l o u d s _ m o d e l 	*/
/*	----------------------------------------------------	*/
public int rest_occi_easiclouds_model(FILE * fh,struct easiclouds_model * sptr,char * prefix, char * nptr)
{
	struct xml_element * wptr;
	if (!( sptr )) return(0);
	fprintf(fh,"POST /%s/ HTTP/1.1\r\n",nptr);
	fprintf(fh,"Category: %s; scheme='http://scheme.%s.org/occi/%s#'; class='kind';\r\n",nptr,prefix,prefix);
	fprintf(fh,"X-OCCI-Attribute: %s.%s.id='%s'\r\n",prefix,nptr,(sptr->id?sptr->id:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.name='%s'\r\n",prefix,nptr,(sptr->name?sptr->name:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.type='%s'\r\n",prefix,nptr,(sptr->type?sptr->type:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.size='%s'\r\n",prefix,nptr,(sptr->size?sptr->size:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.number='%s'\r\n",prefix,nptr,(sptr->number?sptr->number:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.inputs='%s'\r\n",prefix,nptr,(sptr->inputs?sptr->inputs:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.outputs='%s'\r\n",prefix,nptr,(sptr->outputs?sptr->outputs:""));
	fprintf(fh,"X-OCCI-Attribute: %s.%s.instances='%u'\r\n",prefix,nptr,sptr->instances);
	fprintf(fh,"X-OCCI-Attribute: %s.%s.state='%u'\r\n",prefix,nptr,sptr->state);
	return(0);

}

#endif	/* _easiclouds_model_ceasiclouds_model_c_ */
