...
/*	------------------------------------	*/
/*	l i b e r a t e _ p a _ c o n f i g 	*/
/*	------------------------------------	*/
public struct pa_config * liberate_pa_config(struct pa_config * sptr)
{
	if ( sptr )
	{
		if ( sptr->id )
	...
	return((struct pa_config *) 0);

}

/*	------------------------------	*/
/*	r e s e t _ p a _ c o n f i g 	*/
/*	------------------------------	*/
public struct pa_config * reset_pa_config(struct pa_config * sptr)
{
	if ( sptr )
	{
		sptr->id = (char*) 0;
...
/*	------------------------------------	*/
/*	a l l o c a t e _ p a _ c o n f i g 	*/
/*	------------------------------------	*/
public struct pa_config * allocate_pa_config()
{
	struct pa_config * sptr;
	if (!( sptr = allocate( sizeof( struct pa_config ) ) ))
		return( sptr );
	else	return( reset_pa_config(sptr) );
}

/*	------------------------------	*/
/*	x m l i n _ p a _ c o n f i g 	*/
/*	------------------------------	*/
public int xmlin_pa_config(struct pa_config * sptr,struct xml_element * eptr)
{
	...
	for ( wptr=eptr->first; wptr != (struct xml_element *) 0; wptr=wptr->next)
	{
		...
		else if (!( strcmp(wptr->name,"certificate") ))
		{
...
/*	--------------------------------------	*/
/*	r e s t _ o c c i _ p a _ c o n f i g 	*/
/*	--------------------------------------	*/
public int rest_occi_pa_config(FILE * fh,struct pa_config * sptr,char * prefix, char * nptr)
{
	struct xml_element * wptr;
	if (!( sptr )) return(0);
