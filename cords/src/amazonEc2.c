...
public struct amazonEc2 * liberate_amazonEc2(struct amazonEc2 * sptr)
{
	if ( sptr )
	{
		...
		if ( sptr->placementgroup )
			 sptr->placementgroup = liberate(sptr->placementgroup);
		if ( sptr->publicaddr )
...
public int xmlin_amazonEc2(struct amazonEc2 * sptr,struct xml_element * eptr)
{
	...
	for ( wptr=eptr->first; wptr != (struct xml_element *) 0; wptr=wptr->next)
	{
		...
		else if (!( strcmp(wptr->name,"placementgroup") ))
		{
			if ( wptr->value ) { sptr->placementgroup = allocate_string(wptr->value); }
		}
...
public int rest_occi_amazonEc2(FILE * fh,struct amazonEc2 * sptr,char * prefix, char * nptr)
{
	...
	fprintf(fh,"X-OCCI-Attribute: %s.%s.placementgroup='%s'\r\n",prefix,nptr,(sptr->placementgroup?sptr->placementgroup:""));
