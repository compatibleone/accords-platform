...
/*	------------------------------------	*/
/*	l i b e r a t e _ d c _ c o n f i g 	*/
/*	------------------------------------	*/
public struct dc_config * liberate_dc_config(struct dc_config * sptr)
{
	if ( sptr )
	{
		if ( sptr->id )
...
/*	------------------------------	*/
/*	r e s e t _ d c _ c o n f i g 	*/
/*	------------------------------	*/
public struct dc_config * reset_dc_config(struct dc_config * sptr)
{
	if ( sptr )
	{
		sptr->id = (char*) 0;
...
/*	------------------------------------	*/
/*	a l l o c a t e _ d c _ c o n f i g 	*/
/*	------------------------------------	*/
public struct dc_config * allocate_dc_config()
{
	struct dc_config * sptr;
	if (!( sptr = allocate( sizeof( struct dc_config ) ) ))
		return( sptr );
	else	return( reset_dc_config(sptr) );
}

/*	------------------------------	*/
/*	x m l i n _ d c _ c o n f i g 	*/
/*	------------------------------	*/
public int xmlin_dc_config(struct dc_config * sptr,struct xml_element * eptr)
{
...
/*	--------------------------------------	*/
/*	r e s t _ o c c i _ d c _ c o n f i g 	*/
/*	--------------------------------------	*/
public int rest_occi_dc_config(FILE * fh,struct dc_config * sptr,char * prefix, char * nptr)
{
	struct xml_element * wptr;
	if (!( sptr )) return(0);
