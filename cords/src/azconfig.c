...
/*	------------------------------------	*/
/*	l i b e r a t e _ a z _ c o n f i g 	*/
/*	------------------------------------	*/
public struct az_config * liberate_az_config(struct az_config * sptr)
{
	if ( sptr )
	{
		if ( sptr->id )
...
/*	------------------------------	*/
/*	r e s e t _ a z _ c o n f i g 	*/
/*	------------------------------	*/
public struct az_config * reset_az_config(struct az_config * sptr)
{
	if ( sptr )
	{
		sptr->id = (char*) 0;
...
/*	------------------------------------	*/
/*	a l l o c a t e _ a z _ c o n f i g 	*/
/*	------------------------------------	*/
public struct az_config * allocate_az_config()
{
	struct az_config * sptr;
	if (!( sptr = allocate( sizeof( struct az_config ) ) ))
		return( sptr );
	else	return( reset_az_config(sptr) );
}

/*	------------------------------	*/
/*	x m l i n _ a z _ c o n f i g 	*/
/*	------------------------------	*/
public int xmlin_az_config(struct az_config * sptr,struct xml_element * eptr)
{
...
/*	--------------------------------------	*/
/*	r e s t _ o c c i _ a z _ c o n f i g 	*/
/*	--------------------------------------	*/
public int rest_occi_az_config(FILE * fh,struct az_config * sptr,char * prefix, char * nptr)
{
	struct xml_element * wptr;
	if (!( sptr )) return(0);
