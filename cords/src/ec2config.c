...
/*	------------------------------------	*/
/*	l i b e r a t e _ e c 2 c o n f i g 	*/
/*	------------------------------------	*/
public struct ec2config * liberate_ec2config(struct ec2config * sptr)
{
	if ( sptr )
	{
		if ( sptr->id )
...
/*	------------------------------	*/
/*	r e s e t _ e c 2 c o n f i g 	*/
/*	------------------------------	*/
public struct ec2config * reset_ec2config(struct ec2config * sptr)
{
	if ( sptr )
	{
		sptr->id = (char*) 0;
...
/*	------------------------------------	*/
/*	a l l o c a t e _ e c 2 c o n f i g 	*/
/*	------------------------------------	*/
public struct ec2config * allocate_ec2config()
{
	struct ec2config * sptr;
	if (!( sptr = allocate( sizeof( struct ec2config ) ) ))
		return( sptr );
	else	return( reset_ec2config(sptr) );
}

/*	------------------------------	*/
/*	x m l i n _ e c 2 c o n f i g 	*/
/*	------------------------------	*/
public int xmlin_ec2config(struct ec2config * sptr,struct xml_element * eptr)
{
...
/*	--------------------------------------	*/
/*	r e s t _ o c c i _ e c 2 c o n f i g 	*/
/*	--------------------------------------	*/
public int rest_occi_ec2config(FILE * fh,struct ec2config * sptr,char * prefix, char * nptr)
{
	struct xml_element * wptr;
	if (!( sptr )) return(0);
...
#endif	/* _ec2config_cec2config_c_ */
