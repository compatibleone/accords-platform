...
public struct cords_invoice * liberate_cords_invoice(struct cords_invoice * sptr)
{
	if ( sptr )
	{
		...
		if ( sptr->processing )
			 sptr->processing = liberate(sptr->processing);
		sptr = liberate( sptr );
	}
...
public int xmlin_cords_invoice(struct cords_invoice * sptr,struct xml_element * eptr)
{
	...
	for ( wptr=eptr->first; wptr != (struct xml_element *) 0; wptr=wptr->next)
	{
		...
		else if (!( strcmp(wptr->name,"processing") ))
		{
			if ( wptr->value ) { sptr->processing = allocate_string(wptr->value); }
		}
...
public int rest_occi_cords_invoice(FILE * fh,struct cords_invoice * sptr,char * prefix, char * nptr)
{
	...
	fprintf(fh,"X-OCCI-Attribute: %s.%s.processing='%s'\r\n",prefix,nptr,(sptr->processing?sptr->processing:""));
