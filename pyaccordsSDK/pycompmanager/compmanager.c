...
int generateCategoryActionCfile(char *categoryName,listc categoryAtr,listc categoryAct, int flag, char pathf[])
{
 ...
 sprintf(name,"%s/%s/%s/%sAction.c",pathf,PYACCORDS,PYACCORDSG,categoryName);
 ...
 else
 {
    ...
    fprintf(f,"#include \"occi.h\"\n");
    fprintf(f,"#include \"../pysrc/ctools.h\"\n");
    fprintf(f,"#include \"%s.h\"\n",categoryName);
    fprintf(f,"#include <Python.h>\n");
    fprintf(f,"#include \"../pysrc/pytools.h\"\n\n");
...
int generateCategoryActionPyfile(char *categoryName,listc categoryAtr,listc categoryAct,char pathf[])
{
 ...
 sprintf(name1,"%s/%s/%s/%sAct.py",pathf,PYACCORDS,PYACCORDSG,categoryName);
 sprintf(name2,"%s/%s/%s/%sAction.py",pathf,PYACCORDS,PYACCORDSG,categoryName);
 ...
 if((f=fopen(name1,"w"))==NULL)
  ...
  else
  {
   ...
   fprintf(f,"srcdirectory=pypacksrc.srcpydir+\"/pyaccords/pygen/\"\n");
   ...
   while(pelemact)
   {
     ...
     fprintf(f,"\tresCateg,restRes = %s_%s(categoryAtr)\n",categoryName,pelemact->value);
     fprintf(f,"\trestResp = %s + [str(restRes.status),str(restRes.message)]\n",listResult);
  ...
  else
  {
   ...
   fprintf(f,"srcdirectoryc=pypacksrc.srcpydir+\"/pyaccords/pygen/\"\n");
   fprintf(f,"sys.path.append(srcdirectory)\n");
   fprintf(f,"sys.path.append(srcdirectoryc)\n");
   fprintf(f,"from %sClass import *\n",categoryName);
   fprintf(f,"from actionClass import *\n");
   fprintf(f,"\"\"\" Note: respAction is a python class to describe the occi response including a status and a message\n"); 
...
int generateCategoryInterfaceCfile(char *categoryName, listc categoryAtr, int flag, char pathf[])
{
 ...
 sprintf(name,"%s/%s/%s/%sInterface.c",pathf,PYACCORDS,PYACCORDSG,categoryName);
 if((f=fopen(name,"w"))==NULL)
 {
 ...
 else
 {
  ...
  fprintf(f,"#include \"occi.h\"\n");
  fprintf(f,"#include \"../pysrc/ctools.h\"\n");
  fprintf(f,"#include \"../pysrc/listcateg.h\"\n");
  fprintf(f,"#include \"%s.h\"\n",categoryName);
  fprintf(f,"#include <Python.h>\n");
  fprintf(f,"#include \"../pysrc/pytools.h\"\n\n");
  for(j=0;j<4;j++)
...
int generateCategoryPySourcefile(char *categoryName,listc categoryAtr,char pathf[])
{
 ...
 sprintf(name1,"%s/%s/%s/%sClass.py",pathf,PYACCORDS,PYACCORDSG,categoryName);
 sprintf(name2,"%s/%s/%s/%s.py",pathf,PYACCORDS,PYACCORDSG,categoryName);
 sprintf(name3,"%s/%s/%s/%sInterface.py",pathf,PYACCORDS,PYACCORDSG,categoryName);
 if((f=fopen(name1,"w"))==NULL)
 {
 ...
 else
 {
  ...
  else
  {
   fprintf(f,"###############################################################################\n");
   fprintf(f,"# Accords platform                                                            #\n");
   fprintf(f,"# copyright 2012, Hamid MEDJAHED (hmedjahed@prologue.fr)                      #\n");
   fprintf(f,"# Prologue                                                                    #\n");
   fprintf(f,"#-----------------------------------------------------------------------------#\n");
   fprintf(f,"# Licensed under the Apache License, Version 2.0 (the \"License\");             #\n");
   fprintf(f,"# you may not use this file except in compliance with the License.            #\n");
