...
/*	-------------------------	*/
/*	 liberate_cordscript_env  	*/
/*	-------------------------	*/
private	void	liberate_cordscript_env()
{
	struct	cordscript_value * vptr;
	while ((vptr = valueheap) != (struct cordscript_value *) 0)
	{
		valueheap = vptr->next;
...
private	struct occi_element * cordscript_string_filter( char * wptr )
{
	...
	if (!( dptr = occi_create_element( wptr, sptr ) ))
		return( dptr );
	else if (!( dptr->name = occi_free_unquoted_value( dptr->name ) ))
		return( dptr );
	else if (!( dptr->value = occi_free_unquoted_value( dptr->value ) ))
		return( dptr );
	else	return( dptr );
}
	...
	&&  ((wptr = optr->value) != (struct cordscript_value   *) 0))
	{
		...
		else if (!( strncmp( evalue, "http", strlen("http" ) ) ))
		{
							...
							continue;
						{
							if (!( strcmp( dptr->name, "link" ) ))
							{
								if (!( lptr = allocate_string( dptr->value ) ))
									continue;
								else if (!( lptr = occi_free_unquoted_link( lptr ) ))
									continue;
								else
								{
			...
			else if (!( strcmp( wptr->value, "unlink" ) ))
			{
				...
				/* multiple link deletion */
				/* ---------------------- */
				else if (( zptr = occi_delete_links( evalue, _CORDSCRIPT_AGENT, default_tls() )) != (struct occi_response *) 0)
					zptr = occi_remove_response( zptr );
			}
...
private	int	crop_matrix_element( struct cordscript_value * rptr, struct cordscript_value * sptr )
{
	...
	*vptr = 0;
	...
	if (!( rptr->value = allocate_string( mptr ) ))
		return(0);
	else if (!( rptr->value = occi_free_unquoted_value( rptr->value ) ))
		return( 0 );
...
private	int	crop_structure_element( struct cordscript_value * nptr, struct cordscript_value * rptr, struct cordscript_value * sptr )
{
	...
	*vptr = 0;
	...
	if (!( nptr->value = allocate_string( mptr ) ))
		return(0);
	else if (!( nptr->value = occi_free_unquoted_value( nptr->value ) ))
		return( 0 );
