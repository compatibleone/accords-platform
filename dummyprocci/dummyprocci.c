...
/* -------------------------------------------------------------------- */
#ifndef	_dummyprocci_c
#define	_dummyprocci_c
...
#include "dummycontract.c"

/*	---------------------------------------------------------------	*/  
/*	d u m m y _ c o n f i g u r a t i o n               */
/*	---------------------------------------------------------------	*/  
/*	this function loads dummyprocci configuration                      */
/*	from the xml configuration file.                                */
/*	---------------------------------------------------------------	*/  
private	void	dummyprocci_configuration()
{
	load_accords_configuration( &Configuration, "dummyprocci" );
	return;
}

/*	---------------------------------------------------------------	*/  
/*		d u m m y _ b a n n e r                                 */
/*	---------------------------------------------------------------	*/  
/*	this function will be called to display module identification 	*/
/*	when launched from the command line without any parameters.     */
/*	---------------------------------------------------------------	*/  
private	int	dummyprocci_banner()
{
	printf("\n   CompatibleOne Dummy PROCCI : Version 1.0a.0.01");
...
/*	------------------------------------------- */
/*		d u m m y _ i n i t i a l i s e     */
/*	------------------------------------------- */

private	struct rest_server * dummyprocci_initialise(  void * v,struct rest_server * sptr )
{
...
/*	-------------------------------------------	*/
/*	d u m m y _ a u t h o r i s e           */
/*	------------------------------------------- */
private	int	dummyprocci_authorise(  void * v,struct rest_client * cptr, char * username, char * password )
{
...
/*	------------------------------------------- */
/*	d u m m y _ e x t e n s i o n   */
/*	-------------------------------------------	*/
private	struct rest_extension * dummyprocci_extension( void * v,struct rest_server * sptr, struct rest_extension * xptr)
{
	return( xptr );
}

/*	-------------------------------------------	*/
/*	dummyprocci C R U D Method  Extension Functions*/
/*	-------------------------------------------	*/
/* 	   c r e a t e _ d u m m y      */
/*	-------------------------------------------	*/
...
/*	--------------------------------------------	*/
private	struct	occi_interface	dummyprocci_interface = {
	create_dummy,
	retrieve_dummy,
	update_dummy,
	delete_dummy
...
/*	--------------------------------------------	*/
/*	d u m m y _ o p e r a t i o n 		*/
/*	--------------------------------------------	*/
/*	environment and category preparation		    */
/*	--------------------------------------------	*/
private	int	dummyprocci_operation( char * nptr )
{
	...
	set_autosave_cords_xlink_name("links_dummyprocci.xml");
	...
	/* ---------------------------------------- */
	if (!( optr = occi_dummy_config_builder( Configuration.domain, "dummy_configuration" ) ))
		return( 27 );
	...
	if (!( optr = occi_dummy_builder( Configuration.domain, "dummy" ) ))
		return( 27 );
	else if (!( optr->previous = last ))
		first = optr;
	else	optr->previous->next = optr;
	last = optr;
	optr->callback  = &dummyprocci_interface;
	/* -------------------------------------------------------------- */
	...
	/* ---------------------------------------------*/
	if (!( optr = occi_add_action( optr, _CORDS_START, "", start_dummy)))
		return( 27 );
	else if (!( optr = occi_add_action( optr, _CORDS_STOP," ", stop_dummy)))
		return( 27 );
	else if (!( optr = occi_add_action( optr, _CORDS_SAVE, "", save_dummy)))
		return( 27 );
...
/*	------------------------------------------- */
/*	d u m m y _ o p t i o n s       */
/*	------------------------------------------- */
/*	Command line option analysis                */
/*	------------------------------------------- */
private	int	dummyprocci_options(int argc, char * argv[] )
{
	int	status=0;
	int	argi=0;
	char *	aptr;
	dummyprocci_configuration();
	while ( argi < argc )
	{
		if (!( aptr = argv[++argi] ))
			break;
		...
		else if (!( status = dummyprocci_operation(aptr) ))
			continue;
		else	break;
	}
	return(status);
...
{
	if ( argc == 1 )
		return( dummyprocci_banner() );
	else	return( dummyprocci_options( argc, argv ) );
}

        /* ----------  */
#endif 	/* _dummyprocci_c */
        /* ----------  */
